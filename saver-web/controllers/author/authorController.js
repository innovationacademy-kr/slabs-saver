...
const indexPage = async (req, res) => {
  ...
  const category =
    !req.query.category || req.query.category === '0' ? CATEGORY.ALL : +req.query.category;
  ...
  if (position === POSITION.ADMIN) {
    ...
    const articlesData = JSON.stringify(
      articles.map((item) => pick(item, ['id', 'pm7', 'am7', 'status'])),
    );
    ...
    variable = {
      ...
      currentUser,
      articlesData,
      filterCategory: isNaN(category) ? 0 : category,
...
const myPage = async (req, res) => {
  ...
  const viewOption = {
    ...
    table_editing:
      [POSITION.REPOTER, POSITION.DESK, POSITION.CHIEF_EDITOR].indexOf(position) !== -1,
